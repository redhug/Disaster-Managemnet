...
const signUp = (req, res) => {
  ...
  NewUser.findOne({ email: req.body.email }).then(async newUser => {
    if (newUser) {
      return res
        .status(400)
        .json({ email: "Request already sent from this email" });
    } else {
...
const rejectUser = async (req, res) => {
  ...
  readHTMLFile(__dirname + "/userRejected.html", function(err, html) {
    ...
    var template = handlebars.compile(html);
    var replacements = {
      username: req.body.params.name,
      type: "User Rejected",
      message1:
        "Your request has been rejected! The reason is : " +
        req.body.params.rejectedReason
    };
    const transporter = nodemailer.createTransport({
      service: "Gmail",
      auth: {
      ...
      User.findOne(
        ...
        (error, user) => {
          ...
          if (!user) {
            return res.json({
              code: 404,
              message: "This link is not valid or has already expired."
            });
          }
          ...
          bcrypt.compare(body.password, user.password, (err, match) => {
            ...
            if (!match) {
              ...
              return res.json({
                code: 404,
                message: "Cannot use previous passwords"
              });
            }
          });
        }
